...
public final class ModFluids {
	...
	public static final DeferredRegister<FluidType> FLUID_TYPES = DeferredRegister.create(NeoForgeRegistries.FLUID_TYPES, BiomancyMod.MOD_ID);
	public static final DeferredRegister<Fluid> FLUIDS = DeferredRegister.create(BuiltInRegistries.FLUID, BiomancyMod.MOD_ID);

	public static final DeferredHolder<FluidType, ?> ACID_TYPE = registerType("acid", properties -> properties.density(1024).viscosity(1024));
	public static final DeferredHolder<Fluid, ?> ACID = register("acid", AcidFluid.Source::new);
	public static final DeferredHolder<Fluid, ?> FLOWING_ACID = register("flowing_acid", AcidFluid.Flowing::new);
	...
	private static <T extends Fluid> DeferredHolder<Fluid, ?> register(String name, Supplier<T> factory) {
		return FLUIDS.register(name, factory);
	}

	private static DeferredHolder<FluidType, ?> registerTintedType(String name, int colorARGB, UnaryOperator<FluidType.Properties> operator) {
		return FLUID_TYPES.register(name, () -> new TintedFluidType(operator.apply(createFluidTypeProperties()), colorARGB));
	}

	private static DeferredHolder<FluidType, ?> registerType(String name, UnaryOperator<FluidType.Properties> operator) {
