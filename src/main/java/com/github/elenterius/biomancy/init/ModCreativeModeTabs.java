...
public final class ModCreativeModeTabs {
	...
	private static CreativeModeTab.Builder mainTab(CreativeModeTab.Builder builder) {
		return builder
				...
				HolderLookup.RegistryLookup<Enchantment> enchantmentRegistry = params.holders().lookupOrThrow(Registries.ENCHANTMENT);
				for (ResourceKey<Enchantment> key : ModEnchantments.ENCHANTMENT_KEYS) {
					Holder<Enchantment> holder = enchantmentRegistry.getOrThrow(key);
	...
	private static CreativeModeTab.Builder alchemyTab(CreativeModeTab.Builder builder) {
		return builder
				...
				.displayItems((params, output) -> {
					...
					output.accept(createVanillaPotion(Items.POTION, ModPotions.PRIMORDIAL_INFESTATION));
					output.accept(createVanillaPotion(Items.SPLASH_POTION, ModPotions.PRIMORDIAL_INFESTATION));
					output.accept(createVanillaPotion(Items.LINGERING_POTION, ModPotions.PRIMORDIAL_INFESTATION));

					PotionSerumItem potionSerumItem = (PotionSerumItem) ModItems.POTION_SERUM.get();
					for (Potion potion : PotionSerumRecipes.POTIONS) {
						output.accept(potionSerumItem.getInstanceFrom(potion));
					}
	...
	private static CreativeModeTab.Builder devTab(CreativeModeTab.Builder builder) {
		return builder
				...
				.displayItems((params, output) -> {
					...
					ItemStack devChestStack = ModBlocks.FLESHKIN_CHEST.get().asItem().getDefaultInstance();
					devChestStack.set(DataComponents.CUSTOM_NAME, ComponentUtil.literal("[TEST/other_owner] ").append(devChestStack.getHoverName()));
					CompoundTag ownerTag = new CompoundTag();
					ownerTag.putUUID(OwnableEntityBlock.NBT_KEY_OWNER, Util.NIL_UUID);
					BlockItem.setBlockEntityData(devChestStack, ModBlockEntities.FLESHKIN_CHEST.get(), ownerTag);
					output.accept(devChestStack);
	...
	private static ItemStack createVanillaPotion(Item item, Holder<Potion> potion) {
				...
				.displayItems((params, output) -> {
					...
					if (level != null) {
						...
						templates.forEach(id -> {
							...
							stack.set(net.minecraft.core.component.DataComponents.CUSTOM_NAME, ComponentUtil.literal(id.toString()));
							...
							BlockItem.setBlockEntityData(stack, net.minecraft.world.level.block.entity.BlockEntityType.STRUCTURE_BLOCK, tag);
