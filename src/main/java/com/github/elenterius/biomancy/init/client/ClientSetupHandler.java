...
public final class ClientSetupHandler {
	...
	private static void registerSimpleExplosionEmitter(RegisterParticleProvidersEvent event, DeferredHolder<ParticleType<?>, SimpleParticleType> emitter, DeferredHolder<ParticleType<?>, SimpleParticleType> particle) {
	...
	private static void registerItemModelProperties() {
		...
		ItemProperties.register(ModItems.THORN_SHIELD.get(), ResourceLocation.parse("blocking"), shieldPropertyFunc);

		ItemProperties.register(ModItems.CAUSTIC_GUNBLADE.get(), ResourceLocation.parse("melee"), (stack, level, livingEntity, seed) -> GunbladeItem.GunbladeMode.from(stack) == GunbladeItem.GunbladeMode.MELEE ? 1f : 0f);
	}
	...
	public static void onItemColorRegistry(final RegisterColorHandlersEvent.Item event) {
		event.register((stack, tintIndex) -> ((EssenceItem) ModItems.ESSENCE.get()).getColor(stack, tintIndex) | 0xFF000000, ModItems.ESSENCE.get());
		event.register((stack, index) -> index == 1 ? IClientFluidTypeExtensions.of(((BucketItem) stack.getItem()).content).getTintColor() | 0xFF000000 : 0xFF_FFFFFF, ModItems.ACID_BUCKET.get());
		event.register((stack, tintIndex) -> BiometricMembraneBlock.getTintColor(stack, tintIndex) | 0xFF000000, ModItems.BIOMETRIC_MEMBRANE.get());
		event.register((stack, tintIndex) -> ((PotionSerumItem) ModItems.POTION_SERUM.get()).getTintColor(stack, tintIndex) | 0xFF000000, ModItems.POTION_SERUM.get());
	}
	...
	public static void registerGameOverlays(RegisterGuiLayersEvent event) {
		event.registerAbove(VanillaGuiLayers.CAMERA_OVERLAYS, BiomancyMod.rl("frenzy"), ScreenOverlays.FRENZY_OVERLAY);
		event.registerBelow(VanillaGuiLayers.CAMERA_OVERLAYS, BiomancyMod.rl("inside_block"), ScreenOverlays.INSIDE_BLOCK_OVERLAY);
