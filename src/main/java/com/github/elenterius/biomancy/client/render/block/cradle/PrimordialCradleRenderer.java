...
public class PrimordialCradleRenderer extends CustomGeoBlockRenderer<PrimordialCradleBlockEntity> {
	...
	@Override
	public void preRender(PoseStack poseStack, PrimordialCradleBlockEntity animatable, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int color) {
	...
	@Override
	public void renderCubesOfBone(PoseStack stack, GeoBone bone, VertexConsumer buffer, int packedLight, int packedOverlay, int color) {
			...
			if (lifeEnergyPct > 0) {
				...
				if (!bone.isHidden()) {
					for (GeoCube cube : bone.getCubes()) {
						stack.pushPose();
						renderCube(stack, cube, mbs.getBuffer(RenderType.eyes(getTextureLocation(animatable))), 0xf000f0, OverlayTexture.NO_OVERLAY, color);
						stack.popPose();
					}
				}
				isSpecialCube = false;
		...
		else super.renderCubesOfBone(stack, bone, buffer, packedLight, packedOverlay, color);
	}

	@Override
	public void createVerticesOfQuad(GeoQuad quad, Matrix4f matrix4f, Vector3f normal, VertexConsumer buffer, int packedLight, int packedOverlay, int color) {
		...
		if (isSpecialCube && quad.direction() == Direction.NORTH) {
			...
			for (GeoVertex vertex : vertices) {
				...
				if (isTopVertex) {
					createVertex(matrix4f, normal, buffer, packedLight, packedOverlay, color, vertex, textureV, positionY);
				}
				else {
					createVertex(matrix4f, normal, buffer, packedLight, packedOverlay, color, vertex);
				}
			}
		}
		else {
	...
	private void createVertex(Matrix4f matrix4f, Vector3f normal, VertexConsumer buffer, int packedLight, int packedOverlay, int color, GeoVertex geoVertex) {
		vertexPosition.set(geoVertex.position().x(), geoVertex.position().y(), geoVertex.position().z(), 1);
		matrix4f.transform(vertexPosition);
		buffer.addVertex(vertexPosition.x(), vertexPosition.y(), vertexPosition.z())
				.setColor(color)
				.setUv(geoVertex.texU(), geoVertex.texV())
				.setOverlay(packedOverlay)
				.setLight(packedLight)
				.setNormal(normal.x(), normal.y(), normal.z());
	}

	private void createVertex(Matrix4f matrix4f, Vector3f normal, VertexConsumer buffer, int packedLight, int packedOverlay, int color, GeoVertex geoVertex, float v, float y) {
		vertexPosition.set(geoVertex.position().x(), y, geoVertex.position().z(), 1);
		matrix4f.transform(vertexPosition);
		buffer.addVertex(vertexPosition.x(), vertexPosition.y(), vertexPosition.z())
				.setColor(color)
				.setUv(geoVertex.texU(), v)
				.setOverlay(packedOverlay)
				.setLight(packedLight)
				.setNormal(normal.x(), normal.y(), normal.z());
	}
