...
class BioForgeScreenController {
	...
	public BioForgeScreenController(Minecraft minecraft, BioForgeMenu menu) {
		...
		tabs = ModBioForgeTabs.REGISTRY.get().stream().sorted(CATEGORY_COMPARATOR).toList();
	...
	public List<BioForgingRecipe> getOrderedRecipes(RecipeCollection recipeCollection) {
		List<RecipeHolder<?>> list = new ArrayList<>(recipeCollection.getDisplayRecipes(true));
		if (!getPlayer().getRecipeBook().isFiltering(ModRecipeBookTypes.bioForge())) {
			list.addAll(recipeCollection.getDisplayRecipes(false));
		}
		return list.stream().map(holder -> (BioForgingRecipe) holder.value()).toList();
	}
	...
	public boolean isCraftable(int gridIndex) {
		if (gridIndex >= GRID_SIZE) throw new IndexOutOfBoundsException(gridIndex);
		RecipeCollection collection = shownRecipes.get(startIndex + gridIndex);
		return collection.isCraftable(collection.getRecipes().get(0));
	}
	...
	public RecipeHolder<BioForgingRecipe> getRecipeHolderByGameIndex(int index) {
		if (index >= shownRecipes.size()) throw new IndexOutOfBoundsException(index);
		List<RecipeHolder<?>> holders = shownRecipes.get(index).getRecipes();
		if (holders.isEmpty()) throw new IllegalStateException("Empty recipe collection at index " + index);
		return (RecipeHolder<BioForgingRecipe>) holders.get(0);
	}

	public RecipeHolder<BioForgingRecipe> getRecipeHolderByGrid(int gridIndex) {
		if (gridIndex >= GRID_SIZE) throw new IndexOutOfBoundsException(gridIndex);
		return getRecipeHolderByGameIndex(startIndex + gridIndex);
	}
	...
	public boolean isSelectedRecipeVisible() {
		...
		if (recipeSelection.recipe() != null && (activeTab == 0 || getCurrentCategory() == recipeSelection.recipe().getTab())) {
			...
			for (int gridIndex = 0; gridIndex < maxRecipes; gridIndex++) {
				if (getRecipeHolderByGameIndex(startIndex + gridIndex).id().equals(recipeSelection.getRecipeId())) {
	...
	void setSelectedRecipe(int gridIndex) {
		...
		RecipeHolder<BioForgingRecipe> recipeHolder = getRecipeHolderByGameIndex(recipeIndex);
	...
	private static void canCraftRecipe(RecipeCollection recipeCollection, ItemStackCounter itemCounter, RecipeBook book, boolean isCreativePlayer) {
		...
		for (RecipeHolder<?> holder : recipeCollection.getRecipes()) {
			Recipe<?> recipe = holder.value();
			boolean isRecipeKnown = isCreativePlayer || book.contains(holder);
			boolean canCraftRecipe = recipe.canCraftInDimensions(0, 0) && isRecipeKnown;

			if (canCraftRecipe) {
				fitDimensions.add(holder);
			}
			else {
				fitDimensions.remove(holder);
			}

			if (recipe instanceof BioForgingRecipe bioForgingRecipe) {
				if (canCraftRecipe && bioForgingRecipe.isCraftable(itemCounter)) {
					craftable.add(holder);
				}
				else {
					craftable.remove(holder);
				}
			}
			else {
				craftable.remove(holder);
	...
	private void updateAndSearchRecipes() {
		...
		if (!currentSearchString.isEmpty()) {
			ObjectSet<RecipeCollection> searchResult = new ObjectLinkedOpenHashSet<>(Objects.requireNonNull(minecraft.getConnection()).searchTrees().recipes().search(currentSearchString));
			recipes.removeIf(recipeCollection -> !searchResult.contains(recipeCollection));
		}

		if (recipeBook.isFiltering(ModRecipeBookTypes.bioForge())) {
	...
	record RecipeSelection(@Nullable RecipeHolder<BioForgingRecipe> recipeHolder, int tab, int index) {
		public static final RecipeSelection EMPTY = new RecipeSelection(null, -1, -1);

		@Nullable
		public BioForgingRecipe recipe() {
