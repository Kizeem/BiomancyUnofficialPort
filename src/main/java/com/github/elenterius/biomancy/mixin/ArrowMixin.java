...
public abstract class ArrowMixin {
	...
	private boolean onlyApplyEffectIfAllowed(LivingEntity target, MobEffectInstance effectInstance, Entity source) {
		return StatusEffectHandler.canApplySplashEffectIfAllowed(effectInstance.getEffect().value(), target, StatusEffectHandler.CONSUME_ONE_NUTRIENT_PER_ARMOR_PIECE);
	}
