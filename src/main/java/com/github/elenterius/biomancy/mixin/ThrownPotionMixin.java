...
public abstract class ThrownPotionMixin {
	...
	private boolean onlyApplyEffectIfAllowed(LivingEntity livingEntity, MobEffectInstance effectInstance, Entity source) {
		return StatusEffectHandler.canApplySplashEffectIfAllowed(effectInstance.getEffect().value(), livingEntity, StatusEffectHandler.CONSUME_ONE_NUTRIENT_PER_ARMOR_PIECE);
	}
