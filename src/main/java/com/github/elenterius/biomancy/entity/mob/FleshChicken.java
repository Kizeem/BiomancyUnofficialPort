...
public class FleshChicken extends Chicken implements RangedAttackMob, GeoEntity {
	...
	public boolean canBeAffected(MobEffectInstance effectInstance) {
		if (ModMobEffectTags.forgeIsAcid(effectInstance.getEffect().value())) return false;
		...
		if (item == Items.EGG) {
			if (level() instanceof ServerLevel serverLevel) {
				if (random.nextFloat() <= 0.4f) {
					((AcidSplatterBlock) ModBlocks.ACID_SPLATTER.get()).placeSmallSplatter(serverLevel, blockPosition(), Direction.UP, random);
					return null;
				}
				else {
