...
public class AdulteratedHangryEaterFleshBlob extends EaterFleshBlob implements Enemy, AdulteratedFleshkin {
	...
	protected void updateBaseAttributes(byte size) {
		MobUtil.setAttributeBaseValue(this, Attributes.MAX_HEALTH.value(), size * BASE_MAX_HEALTH);
		MobUtil.setAttributeBaseValue(this, Attributes.MOVEMENT_SPEED.value(), 0.2f + 0.01f * size);
		MobUtil.setAttributeBaseValue(this, Attributes.ARMOR.value(), size * BASE_ARMOR);
		MobUtil.setAttributeBaseValue(this, Attributes.ATTACK_DAMAGE.value(), BASE_ATTACK_DAMAGE * (size * 0.5f));
	}
