...
public class DigesterBlockEntity extends MachineBlockEntity<DigestingRecipe, DigesterStateData> implements MenuProvider, GeoBlockEntity {
	...
	public static final DeferredHolder<RecipeType<?>, ?> RECIPE_TYPE = ModRecipes.DIGESTING_RECIPE_TYPE;
	...
	protected @Nullable DigestingRecipe resolveRecipeFromInput(Level level) {
		return ((com.github.elenterius.biomancy.crafting.recipe.SimpleRecipeType.AdvancedRecipeType<DigestingRecipe>) RECIPE_TYPE.get()).getBestRecipeFor(level, inputInventory.getRecipeWrapper()).orElse(null);
	}
	...
	protected boolean craftRecipe(DigestingRecipe recipeToCraft, Level level) {
		ItemStack result = recipeToCraft.assemble(new ContainerRecipeInput(inputInventory.getRecipeWrapper()), level.registryAccess());
