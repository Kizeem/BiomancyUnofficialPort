...
public abstract class GunItem extends ProjectileWeaponItem implements Gun, KeyPressListener {
	...
	protected static int getBonusReloadReduction(ItemStack stack) {
		return 5 * EnchantmentUtil.getLevel(stack, Enchantments.QUICK_CHARGE);
	}

	protected static float getBonusProjectileDamageModifier(ItemStack stack) {
		return 0.6f * EnchantmentUtil.getLevel(stack, Enchantments.POWER);
	}
	...
	protected static int getBonusProjectileKnockBackModifier(ItemStack stack) {
		return EnchantmentUtil.getLevel(stack, Enchantments.PUNCH);
	}
	...
	public void shoot(ServerLevel level, LivingEntity shooter, InteractionHand usedHand, ItemStack projectileWeapon) {
		...
		if (success) {
			...
			projectileWeapon.hurtAndBreak(getDurabilityCost(projectileWeapon), shooter, usedHand == InteractionHand.MAIN_HAND ? EquipmentSlot.MAINHAND : EquipmentSlot.OFFHAND);
	...
	@Override
	protected void shootProjectile(LivingEntity shooter, Projectile projectile, int index, float velocity, float inaccuracy, float yaw, LivingEntity target) {
	}
	...
	public void onUseTick(Level level, LivingEntity shooter, ItemStack stack, int remainingUseDuration) {
		if (!level.isClientSide && level instanceof ServerLevel serverLevel) {
			...
			if (canShoot) {
				...
				setShootTimestamp(stack, serverLevel.getGameTime());
			}
		}
	}
