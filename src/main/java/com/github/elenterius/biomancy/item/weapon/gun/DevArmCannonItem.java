...
public class DevArmCannonItem extends Item implements GeoItem, IArmPoseProvider, ItemTooltipStyleProvider, KeyPressListener {
	...
	private static byte readProjectileIndex(ItemStack stack) {
		return stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getByte("ProjectileIndex");
	}

	private static void writeProjectileIndex(ItemStack stack, byte index) {
		CompoundTag tag = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
		tag.putByte("ProjectileIndex", index);
	...
	public void onServerReceiveKeyPress(ItemStack stack, ServerLevel level, Player player, byte flags) {
		...
		writeProjectileIndex(stack, flags);
	}
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand usedHand) {
		if (!level.isClientSide) {
			...
			byte index = readProjectileIndex(stack);
			if (index < 0 || index >= ModProjectiles.PRECONFIGURED_PROJECTILES.size()) {
				index = 0;
				writeProjectileIndex(stack, (byte) 0);
			}
