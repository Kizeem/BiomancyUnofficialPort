...
public final class StatusEffectHandler {
	...
	public static void onEffectAdded(final MobEffectEvent.Added event) {
		...
		if (event.getEffectInstance().getEffect() == ModMobEffects.FRENZY) {
	...
	public static void onEffectRemoval(final MobEffectEvent.Remove event) {
		...
		if (event.getEffect() == ModMobEffects.ESSENCE_ANEMIA && ModMobEffectTags.isNotRemovableWithCleansingSerum(ModMobEffects.ESSENCE_ANEMIA.value())) {
	...
	public static void addWithdrawalAfterFrenzy(LivingEntity livingEntity, @Nullable MobEffectInstance removedEffectInstance) {
		...
		if (removedEffectInstance.getEffect() != ModMobEffects.FRENZY) return;
	...
	public static void onFoodEaten(final LivingEntityUseItemEvent.Finish event) {
		...
		if (stack.has(net.minecraft.core.component.DataComponents.FOOD) && stack.is(ModItemTags.SUGARS)) {
	...
	public static boolean canApplySplashEffectIfAllowed(MobEffect effect, LivingEntity target, BiConsumer<LivingArmorItem, ItemStack> nutrientsConsumer) {
		...
		if (target.isInvertedHealAndHarm()) {
			if (effect == MobEffects.HEAL.value()) {
				category = MobEffectCategory.HARMFUL;
			}
			else if (effect == MobEffects.HARM.value()) {
	...
	public static boolean hasAcidEffect(LivingEntity livingEntity) {
		for (Holder<MobEffect> effect : livingEntity.getActiveEffectsMap().keySet()) {
			if (ModMobEffectTags.forgeIsAcid(effect.value())) return true;
		}
		return false;
	}
