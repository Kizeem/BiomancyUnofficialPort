...
public class CorrosiveEffect extends StatusEffect {
	...
	private boolean convertZombieToSkeleton(ServerLevel level, LivingEntity livingEntity) {
		if (livingEntity instanceof Zombie zombie && net.neoforged.neoforge.event.EventHooks.canLivingConvert(zombie, EntityType.SKELETON, null)) {
			...
			if (skeleton != null) {
				...
				net.neoforged.neoforge.event.EventHooks.onLivingConvert(zombie, skeleton);
				if (!zombie.isSilent()) {
		...
		else if (livingEntity instanceof ZombieHorse zombieHorse && net.neoforged.neoforge.event.EventHooks.canLivingConvert(zombieHorse, EntityType.SKELETON_HORSE, null)) {
			...
			if (horse != null) {
				...
				net.neoforged.neoforge.event.EventHooks.onLivingConvert(zombieHorse, horse);
