...
public class DecomposingCategory implements IRecipeCategory<RecipeHolder<DecomposingRecipe>> {
	...
	public static final RecipeType<RecipeHolder<DecomposingRecipe>> RECIPE_TYPE = RecipeType.createRecipeHolderType(ModRecipes.DECOMPOSING_RECIPE_TYPE.getId());
	...
	public void draw(RecipeHolder<DecomposingRecipe> recipeHolder, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
		...
		MutableComponent costString = ComponentUtil.literal("-" + recipe.getCraftingCostNutrients(recipeInput));
