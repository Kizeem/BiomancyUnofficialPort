...
public class BioBrewingCategory implements IRecipeCategory<RecipeHolder<BioBrewingRecipe>> {
	...
	public static final RecipeType<RecipeHolder<BioBrewingRecipe>> RECIPE_TYPE = RecipeType.createRecipeHolderType(ModRecipes.BIO_BREWING_RECIPE_TYPE.getId());
	private final IDrawable background;
	private final IDrawable icon;

	private final StackArrayRecipeInput inputInventory;
	...
	public BioBrewingCategory(IGuiHelper guiHelper) {
		...
		inputInventory = new StackArrayRecipeInput(BioLabBlockEntity.INPUT_SLOTS);
	}
	...
	public void setRecipe(IRecipeLayoutBuilder builder, RecipeHolder<BioBrewingRecipe> recipeHolder, IFocusGroup focuses) {
		...
		ItemStack reactantDisplay = recipe.getReactant().getItems().length > 0 ? recipe.getReactant().getItems()[0].copy() : ItemStack.EMPTY;
		if (!reactantDisplay.isEmpty()) reactantDisplay.setCount(recipe.getReactantCount());
		builder.addSlot(RecipeIngredientRole.INPUT, 55, 10).addItemStack(reactantDisplay);
	...
	public void draw(RecipeHolder<BioBrewingRecipe> recipeHolder, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
		...
		for (int i = 0; i < slotViews.size() && i < inputInventory.size(); i++) {
		...
		int ticks = recipe.getCraftingTimeTicks(new ContainerRecipeInput(inputInventory));
		...
		Component costText = ComponentUtil.literal("-" + recipe.getCraftingCostNutrients(new ContainerRecipeInput(inputInventory)));
