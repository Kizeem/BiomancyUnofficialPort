...
public final class PrimordialEcosystem {
	...
	public static boolean placeMalignantBlocks(ServerLevel level, BlockPos pos, RandomSource random, float chargePct) {
		...
		FleshVeinsBlock veinsBlock = (FleshVeinsBlock) ModBlocks.MALIGNANT_FLESH_VEINS.get();
	...
	public static boolean placeBloomOrBlocks(ServerLevel level, BlockPos pos, Direction direction) {
		...
		if (random.nextFloat() < 0.7f && ((BloomBlock) ModBlocks.PRIMAL_BLOOM.get()).mayPlaceOn(level, pos, state)) {
			...
			if (canBeReplaced && noBloomNearby) {
				BlockState blockState = ((BloomBlock) ModBlocks.PRIMAL_BLOOM.get()).getStateForPlacement(level, relativePos, direction);
