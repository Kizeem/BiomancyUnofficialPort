...
public final class BioForgeTab {
	...
	public static BioForgeTab fromJson(JsonObject json) {
		...
		return ModBioForgeTabs.REGISTRY.get().get(ResourceLocation.parse(categoryId));
	}
	...
	public static BioForgeTab fromNetwork(FriendlyByteBuf buffer) {
		BioForgeTab value = ModBioForgeTabs.REGISTRY.get().get(buffer.readResourceLocation());
