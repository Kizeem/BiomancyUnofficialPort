...
public record MobEffectTribute(int lifeEnergy, int successModifier, int diseaseModifier, int hostileModifier, int anomalyModifier) implements Tribute {
	...
	static Tribute from(ItemStack stack) {
		...
		if (isPotionItem) {
			Iterable<MobEffectInstance> effectInstances = stack.getOrDefault(net.minecraft.core.component.DataComponents.POTION_CONTENTS, PotionContents.EMPTY).getAllEffects();
		...
		if (isSuspiciousStewItem) {
			SuspiciousStewEffects stewEffects = stack.getOrDefault(net.minecraft.core.component.DataComponents.SUSPICIOUS_STEW_EFFECTS, SuspiciousStewEffects.EMPTY);
	...
	public static class Builder {
		...
		public Builder addEffect(MobEffectInstance instance, float multiplier) {
			addEffect(instance.getEffect().value(), instance.getAmplifier(), instance.getDuration(), multiplier);
			return this;
		}
