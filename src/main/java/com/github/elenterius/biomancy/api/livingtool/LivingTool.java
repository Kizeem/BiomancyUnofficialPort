...
public interface LivingTool extends NutrientsContainerItem {
	...
	default boolean isValidEnchantment(ItemStack livingTool, Holder<Enchantment> enchantment) {
		return enchantment.unwrapKey().map(key -> !INVALID_ENCHANTMENTS.contains(key)).orElse(false);
	}
	...
	int getLivingItemAbilityCost(ItemStack livingTool, ItemAbility toolAction);
